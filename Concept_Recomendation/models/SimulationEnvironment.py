...
class explore_environment:
	...
	def sort_topic(self):
		# sort the topic according to the concept similarity

		for topic_index in list(self.topic_groups.keys()):
			
			topic_embedding = torch.stack([self.action_encode[self.concepts.index(i)] for i in self.topic_groups[topic_index]])
			concept_similarity =  cosine_similarity(topic_embedding)

			mask = np.ones(len(self.topic_groups[topic_index]),)
			...
			while True: 
				next_node = (concept_similarity[node] + mask).argmax()

				if mask[next_node] == -1 << 18:
					break
			...
			assert len(path) == len(self.topic_groups[topic_index]), f"Path length {len(path)} does not match topic length {len(self.topic_groups[topic_index])}"
			assert len(set(path)) == len(self.topic_groups[topic_index]), f"Path contains duplicates {path}"

			self.topic_groups[topic_index] = [self.topic_groups[topic_index][i] for i in path]
	...
	def generate_episode(self, index = None):
		...
		#create the batches of images observed on each step of the exploration
		#these batch are consistently created so that the ammount of images observed
		#which are related to a concept are at least "threshold" before the user in 
		# the exploration actually adding the new concept.
		for step in topic:
